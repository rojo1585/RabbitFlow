...
TTL expires → re-deliver

Retries exhausted: NACK requeue=false
...
NotificationBatchHandler

Retry Queue TTL

Resilience flow (client-side retry, no message amplification):

Handler fails → consumer publishes to the appropriate retry queue (via default exchange) and ACKs the original delivery.
Retry queue TTL expires → broker dead-letters back to the main exchange → message re-delivered to the main queue.
Retries exhausted → consumer NACKs with requeue=false → broker dead-letters via x-dead-letter-exchange (a direct DLX bound to the DLQ with routing key "dead").
Poison message (deserialize/upgrade throws) → same retry/DLQ flow via top-level try/catch in OnMessageReceived.
📦 Installation
Install via .NET CLI:
...
RetryDelays: One entry per retry attempt after the first. Must have at least MaxRetries - 1 entries. Default: [0s, 5s, 30s].
...
RoutingKey	string (required)	—	Non-empty
PrefetchCount	ushort	10	≥ 1 (0 deadlocks the batch consumer)
...
Connection drops → driver fires ConnectionShutdownAsync.
OnConnectionShutdown signals a TaskCompletionSource and nulls the connection.
The background loop wakes up, applies exponential backoff (1s → 2s → 4s → ... → MaxBackoffSeconds cap), and creates a new IConnection.
Publishers and consumers re-declare topology idempotently on the new connection.
